...
界面支持简体中文和英文，可在主界面的“通用”区域直接切换，也可以选择跟随系统语言。

程序启动后会打开主界面并显示在 Dock 中，不创建菜单栏状态图标。关闭主界面后窗口和 Dock 图标
都会隐藏，但快捷键监听会继续在后台运行；再次打开应用即可恢复主界面。主界面还可配置登录时
自动启动或彻底退出程序。
...
主界面会持续刷新授权状态；完成授权并返回后会自动启用按键监听，无需重新启动程序。
